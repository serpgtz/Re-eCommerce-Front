...
const BuyForm = ({error, location}) => {
 ...
 console.log(Object.values(input).join("").length)
  ...
  Necesitas elegir la Sucursal — <strong>RETIRAR EN</strong>
      ...
      <TextField error={errorf.name && true} helperText={errorf.name} name='name' onChange={handleOnChange} value={input.name}  label="Nombre" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '25ch', height: '6ch' },
      }} />

      <TextField error={errorf.lastName && true} helperText={errorf.lastName} name='lastName' onChange={handleOnChange} value={input.lastName}  label="Apellido" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '25ch', height: '6ch' },
      }} />

      <TextField error={errorf.email&& true} helperText={errorf.email} name='email' onChange={handleOnChange} value={input.email}  label="Email" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '50ch', height: '6ch' },
      }} />
            
      <TextField error={errorf.phone && true} helperText={errorf.phone} name='phone' onChange={handleOnChange} value={input.phone}  label="Celular" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '50ch', height: '6ch' },
      }} />

       <TextField  name='info' onChange={handleOnChange} value={input.info}  label="Informacion adicional" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '50ch', height: '15ch' },
      }} /> 

    <TextField error={errorf.address_user&& true} name='address_user' helperText={errorf.address_user} onChange={handleOnChange} value={input.address_user}  label="Domicilio" variant="outlined" sx={{
        '& > :not(style)': { m: 1, width: '50ch', height: '6ch'},
      }} /> 

     {!error && Object.values(input).join("").length !== 0 &&  <input type='submit' disabled={ Object.keys(errorf).length !== 0 && true}  onClick={handleClick} value='Finalizar la Orden'></input>
       }
